...
def train_op(model, loader, optimizer, epochs=20):
    model.train()
...
class Client(FederatedTrainingDevice):
    ...
    """
    def compute_weight_update(self, epochs=1, loader=None):
        # Store the previous weights correctly (ensure they're tensors)
        for old, current in zip(self.W_old, self.W):
            if not isinstance(old, torch.Tensor) or not isinstance(
                current, torch.Tensor
            ):
                raise TypeError(
                    f"Expected torch.Tensor but got {type(old)} and {type(current)}"
                )
            old.copy_(current)
        ...
        # Compute weight difference (ensure that we are working with tensors)
        for old, current, diff in zip(self.W_old, self.W, self.dW):
            if (
                not isinstance(old, torch.Tensor)
                or not isinstance(current, torch.Tensor)
                or not isinstance(diff, torch.Tensor)
            ):
                raise TypeError(
                    f"Expected torch.Tensor but got {type(old)}, {type(current)}, {type(diff)}"
                )
            diff.copy_(current - old)
